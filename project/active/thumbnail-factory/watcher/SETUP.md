...
5. 다시 만들려면(문구 수정 등): **생성요청(F) 다시 체크** (승인 자동 해제 후 재생성) — 또는 상태(G) 드롭다운에서 `🔄 재요청` 선택.

> 상태(G)는 **시스템이 자동으로 채우는 칸**(요청됨→생성중→검수대기→완료). 사람은 보통 안 건드리고, 필요 시 드롭다운으로 수동 지정 가능.
> 타이밍: "생성요청(F) 체크 = 생성 시작", "승인(J) 체크 = 적재". 체크 후 ~1분 내 반영(워처 폴링).
